...
int AVCodecBase::setOption(const AVDictionary* dict)
{
...
std::shared_ptr<AVParamWarpper> AVCodecBase::copyCodecParam()
{
	std::shared_ptr<AVParamWarpper> param;
	std::unique_lock<std::mutex> lock(mtx_);
	if (!codec_ctx_)
	{
	...
	std::shared_ptr<AVParamWarpper> warp = std::make_shared<AVParamWarpper>();
	*param->time_base = codec_ctx_->time_base;
	avcodec_parameters_from_context(param->para, codec_ctx_);
	return param;
}

std::shared_ptr<AVCodecParameters> AVCodecBase::copyCodecParameter()
{

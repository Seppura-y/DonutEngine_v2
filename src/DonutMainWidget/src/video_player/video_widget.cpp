...
VideoWidget::VideoWidget(QWidget* parent) : QWidget(parent)
{
	...
	QObject::connect(&timer_mouse_hide_, &QTimer::timeout, this, &VideoWidget::onMouseHideTimeout);
	timer_mouse_hide_.start(1000);
}

VideoWidget::VideoWidget(int row, int colum, int width, int height, QWidget* parent)
{
	this->setWindowTitle("RenderWidget");
	//setAttribute(Qt::WA_TranslucentBackground);
	//setWindowFlags(Qt::FramelessWindowHint | Qt::WindowMinMaxButtonsHint);
...
void VideoWidget::openMediaFile(QString file_path)
{
	if (!render_manager_)
	{
		initVlcManager();
...
void VideoWidget::setMediaPause(bool pause)
{
...
void VideoWidget::setSeekPos(double value)
{
	if (!render_manager_) return;
	render_manager_->setTimePos(value);
}

void VideoWidget::setSoundVolume(int value)
{
	if (!render_manager_) return;
	render_manager_->setSoundVolume(value);
}

void VideoWidget::setMediaStop()
{
	if (render_manager_)
	{
...
void VideoWidget::dragEnterEvent(QDragEnterEvent* ev)
{
	ev->acceptProposedAction();
}

void VideoWidget::dropEvent(QDropEvent* ev)
{
...
void VideoWidget::initVlcManager()
{
...
void VideoWidget::onMouseHideTimeout()
{
